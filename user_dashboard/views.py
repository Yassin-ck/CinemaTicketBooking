...
from .serializers import TicketBookingCreateUpdateSerializer
...
def screen_seat_details(Q_Base):
    return (
        ScreenSeatArrangement.objects.filter(Q_Base)
        .annotate(
            ...
            language=F("screen__shows__language__name"),
        )
        .values(
            "seating",
            "screen_number",
...
class MovieSearching(APIView):
    name = openapi.Parameter(
        ...
        description="movie name or director name",
    ...
    @swagger_auto_schema(
        ...
        responses={200: MovieDetailListSerializer, 404: "Not Found", 500: "errors"},
    )
    def get(self, reqeust, hi=None):
        q = reqeust.GET.get("q")
        queryset = MoviesDetails.objects.filter(
            Q(movie_name__icontains=q) | Q(director__icontains=q)
...
class MovieSelectionView(APIView):
    ...
    movie = openapi.Parameter(
        "movie",
        in_=openapi.IN_QUERY,
        description="movies",
        type=openapi.TYPE_STRING,
    )
    cinemas = openapi.Parameter(
        "cinemas",
        in_=openapi.IN_QUERY,
        description="theatre",
        type=openapi.TYPE_STRING,
    )
    screen = openapi.Parameter(
        "screen",
        in_=openapi.IN_QUERY,
    ...
    def get(self, request):
        ...
        if movie and cinemas and screen:
            if q != "all" and q is not None:
                response = self.get_screen_details(
                    location, cinemas, date, screen, movie, times, q
                )
            else:
                response = self.get_screen_details(
                    location, cinemas, date, screen, movie, times
                )
            ...
            for key, value in all_data.items():
                if (
                    key.startswith(CACHE_PREFIX_TICKET_BOOKING)
                    and response["show_time"] == value.get("time")
                    and str(response["show_dates"]) == str(value.get("date"))
                    and response["theatre_name"] == value.get("theatre_name")
                    and str(response["screen_number"])
                    == str(value.get("screen_number"))
                ):
                    print(value)
                    for ticket in set(value.get("tickets", [])):
                        index = (
                            row_alpha.index(ticket[0])
                            if ticket[0] in row_alpha
                            else None
                        )
                        if index is not None:
                            response["seating"][index][int(ticket[1:]) - 1] = (
                                f"{ticket}{CACHED}"
                                if request.user.is_authenticated
                                and value["cache_id"] == request.user.email
                                else f"{ticket}{BOOKED}"
                            )
            ...
            return Response(response, status=status.HTTP_200_OK)
        queryset = (
            MoviesDetails.objects.filter(Q_Base)
            .distinct()
            .values("id", "movie_name", "poster", "director", "shows__language__name")
        )
        ...
        return Response(
            {"movies": movie_data, "languages": languages}, status=status.HTTP_200_OK
        )
    ...
    def get_theatre_screen_details(self, Q_Base, date):
        ...
        queryset = (
            ...
            .values(
                "screen_number", "theatre_name", "show_time", "language", "show_dates"
            )
            .distinct()
        )
        ...
        for screen_details in queryset:
            ...
            formatted_date = datetime.strptime(
                str(screen_details.get("show_dates")), "%Y-%m-%d"
            ).strftime("%b %d %a")
        ...
        response_data = {"data": current_screen_data, "dates": date_data}
            ...
            for screen_details in queryset:
                ...
                if str(screen_details.get("show_dates")) == date:
                    existing_screen = next(
                        (
                            screen
                            for screen in current_screen_data
...
class SingleMovieDetailsView(APIView):
    def get(self, request, movie, id):
        queryset = (
            MoviesDetails.objects.filter(Q(movie_name=movie) & Q(id=id))
            .prefetch_related(
                Prefetch("rating_set", Rating.objects.select_related("review", "user"))
            )
            .first()
        )
...
class TicketCachingView(APIView):
    ...
    def put(self, request):
        ...
        if cache_id and request.data["tickets"][0][-1] != BOOKED:
            ...
            return Response(
                {"error": "Already Booked..."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(self, request):
        ...
        if cache_id is not None:
            ...
            show_price = (
                ...
                .values("screen__ticket_rate", "movies__movie_name")
                .first()
            )
            ...
            customer = stripe.Customer.create(
                email=request.user.email, phone=user_profile.phone
            )
        try:
    ...
    def get(self, request):
        ...
        if session_id:
            ...
            if cached_data is None and cached_ticket_data is not None:
                ...
                return Response(
                    {"msg": "session cleared..."}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            cache.delete(key)
            return Response(
                {"error": "Error while payment processing..."},
